...
function lsp_clean_archive_title($title){
  ...
  if($newtitle[1]=="")return $title;
  else return $newtitle[1];
...
function lsp_reports_scripts(){
    ...
    wp_add_inline_script('lsp-reports',"
...
function lsp_project_scripts(){
    ...
    $lsp_language = urlencode(substr(get_bloginfo('language'), 0, 2));
    if($current_screen->post_type == 'localproject')$lsp_cname = __('Client Name / Project Title', 'lsp_text_domain').":<br>";
    if($current_screen->post_type == 'seoabtest')$lsp_cname = __('Title', 'lsp_text_domain').":<br>";
    if($current_screen->post_type == 'localproject') $lsp_dname = 'Your optional description of what you did for the client / who they are goes below -- pictures can be added with the Add Media button. Their-problem/your-solution explanations with search keywords work well for these:';
    ...
    wp_add_inline_script('lsp-project', "jQuery(document).ready(function(){
        jQuery(function() {
          function log( message ) {
            jQuery( '<div>' ).text( message ).prependTo( '#log' );
            jQuery( '#log' ).scrollTop( 0 );
...
function lsp_admin_scripts(){
    $lsp_language = urlencode(substr(get_bloginfo('language'), 0, 2));
     ...
     if (sanitize_text_field($_GET['taxonomy']) && lsp_get_option('lsp-optimize-taxes0') == "yes") wp_add_inline_script('lsp-admin',"
    ...
    if (sanitize_text_field($_GET['user_id']) && lsp_get_option('lsp-optimize-authors0') == "yes") wp_add_inline_script('lsp-admin',"
        jQuery('#wpseo_author_title').parent().parent().parent().hide();
        jQuery('#wordpress-seo').hide();");

    if (lsp_get_option('lsp-preview-urls') != 'yes') wp_add_inline_script('lsp-admin',"
    ...
    if (lsp_get_option('lsp-reviews-name') == '') wp_add_inline_script('lsp-admin',"
  ...
  $lsp_warn5 = __('You need to first fill out and save your basic settings above.','lsp_text_domain');
   ...
   if (lsp_get_option('lsp-services') == '') wp_add_inline_script('lsp-admin',"
        jQuery('#serviceslink3').click(function(event){event.preventDefault();alert('".$lsp_warn5."')})");
    
    wp_add_inline_script('lsp-admin',"jQuery('#lsp-api-state').blur(function(){
...
if($_GET['post_type']=='localproject') wp_add_inline_script('lsp-admin',"
jQuery('#publish').click(function(event){
  ...
  wp_add_inline_script('lsp-admin',"
        ...
        if (sanitize_text_field($_GET['reports'])) wp_add_inline_script('lsp-admin',"
...
function lsp_feedback_scripts(){
    ...
    wp_add_inline_script('lsp-feedback', "
...
{
?>
        <a href="http://www.bestlocalseotools.com" target="_blank"><?php
    _e("Upgrade to Premium", 'lsp_text_domain');
?></a>
<br>

  <b><?php 
  if($plus==false) _e("Local Portfolio SEO Visits So Far This Month:", 'lsp_text_domain' );
  if($plus) _e("Local Portfolio SEO Visits So Far This Subscription Period:", 'lsp_text_domain' );
   ?></b>
<br><?php
...
{
    return wp_kses(get_option('lsp_content'),array( 'a' => array( 'href'=>array(),'title'=>array()) ));
}
...
function lsp_testimonials($atts)
{
  $lsp_sc_holder == "";
  global $wpdb; 
  $result = $wpdb->get_results($wpdb->prepare("SELECT ID from $wpdb->posts, $wpdb->postmeta WHERE post_type='localproject' AND post_status = 'publish' AND meta_key = 'lsp_post_testimonial' AND meta_value != '' AND $wpdb->postmeta.post_id = $wpdb->posts.ID ORDER BY menu_order DESC"));
  $i = 0;
...
function lsp_localportfoliocontent($content)
{
    ...
    if (($post->ID == lsp_get_option('lsp-ptemplate-id')) && $wp_query->query_vars['city'] == '' && (!(strstr($_SERVER['REQUEST_URI'], "feedback")))) {
        $newContent = wp_kses(get_option('lsp_content'),array( 'a' => array( 'href'=>array(),'title'=>array())));
...
{
    $fHolder = "";
    $fHolder = wp_remote_retrieve_body(wp_remote_post($url, array('timeout' => 105, 'body' => array('data',$data))));
    $fHolder = wp_remote_retrieve_body(wp_remote_post($url, array('timeout' => 45, 'body' => array('data',$data))));
    if($fHolder)return $fHolder;  
    return 'There was an error with contacting the service. Please check your Best Local SEO Tools settings like the state *full name* and city name. Some cities may cause bugs because they are not in our database. If that is the case -- please try a different city (center of service area).';    
}
...
//   $query2 = "SELECT a.meta_value as city, b.meta_value as state FROM ".$wpdb->prefix."postmeta a, ".$wpdb->prefix."postmeta b, ".$wpdb->prefix."postmeta c WHERE c.meta_value LIKE '%%".get_option('lsp-keyphrase'.$i)."%%' AND a.meta_key='lsp_post_city' AND b.meta_key='lsp_post_state' AND c.meta_key='lsp_post_tags' AND a.post_id = b.post_id AND b.post_id = c.post_id";
      ...
      foreach ($citystates as $citystate) {
        ...
        if(!(in_array($citystate->city.",".$citystate->state,$projectArray)))$projectsString .= $pstring.",".'US'."|";
        $projectsCounter++;
      }
      echo $projectsString;   
     die();    
...
function lsp_homepage_title(){
  ...
  if(get_option('lsp-auto-adjust')=="yes" && is_front_page()){
    $zstring = '';
    if(get_option('lsp-services'))$zstring = get_option('lsp-services');
    if(get_option('lsp-services2'))$zstring .= ", ". get_option('lsp-services2');
    if(get_option('lsp-services3'))$zstring .= ", ". get_option('lsp-services3');

    $bigCity = get_option('lsp-biggest-city');
    if($bigCity) $bigCity = $bigCity . " ";
    global $post;
    if(get_post_meta($post->ID,'_lsp_post_locktitle', true)!="yes" && get_option('lsp-biztype')!='ls2') echo "<title>". $bigCity . get_option('lsp-api-city'). ", ".get_option('lsp-api-state')." ". ucwords($zstring)."</title>";
    if(get_post_meta($post->ID,'_lsp_post_locktitle', true)!="yes" && get_option('lsp-biztype')=='ls2') echo "<title>". ucwords($zstring). __(' near ','lsp_text_domain') .$bigCity . get_option('lsp-api-city'). ", ".get_option('lsp-api-state')."</title>";
  }

  ////EDITPOINT
  if(get_option('lsp-posts-adjust')!="no" && get_post_type()=='localproject' && get_post_meta($post->ID,'lsp_project_terms',true)!=""){
    if(get_option('lsp-biztype')!='ls2') echo "<title>". get_post_meta($post->ID,'lsp_project_terms',true)."</title>";
    if(get_option('lsp-biztype')=='ls2') echo "<title>". get_post_meta($post->ID,'lsp_project_terms',true)."</title>";
  }
...
function lsp_prescribed_title( $title ) {
  ...
  if(get_option('lsp-ptemplate-id') == $post->ID ){
    ...
    if(get_option('lsp-biztype')=='ls2') return stripslashes(ucwords($currentTerm)) . __(' near ', 'lsp_text_domain') . stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['city'])))) . ", " . stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['state'])))) ."";
    else return "".stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['city'])))) . ", " . stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['state'])))) . " " . stripslashes(ucwords($currentTerm)) ."";
  }  

  if(lsp_get_option('lsp-auto-adjust')=="yes" && is_front_page()){
    $zstring = '';
    if(lsp_get_option('lsp-services'))$zstring = lsp_get_option('lsp-services');
    if(lsp_get_option('lsp-services2'))$zstring .= ", ". lsp_get_option('lsp-services2');
    if(lsp_get_option('lsp-services3'))$zstring .= ", ". lsp_get_option('lsp-services3');
    $bigCity = lsp_get_option('lsp-biggest-city');
    if($bigCity) $bigCity = $bigCity . " ";
    if(get_option('lsp-biztype')!='ls2') echo "<title>". $bigCity . get_option('lsp-api-city'). ", ".get_option('lsp-api-state')." ". ucwords($zstring)."</title>";
    if(get_option('lsp-biztype')=='ls2') echo "<title>". ucwords($zstring). __(' near ','lsp_text_domain') .$bigCity . get_option('lsp-api-city'). ", ".get_option('lsp-api-state')."</title>";
...
function lsp_keyphrase_one_page_title($title)
{
    ...
    if (get_the_ID() == lsp_get_option('lsp-ptemplate-id') && ((lsp_get_option('lsp-keyphrase') == $currentTerm))) {
        ...
        if (($wp_query->query_vars['city'] != "list") && ($wp_query->query_vars['state'] != "list")) {
            ...
            if(lsp_get_option('lsp-biztype')=='ls2') echo "<title>" . stripslashes(ucwords($currentTerm)) . __(' near ', 'lsp_text_domain') . stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['city'])))) . ", " . stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['state'])))) ."</title>";  
            else echo "<title>".stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['city'])))) . ", " . stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['state'])))) . " " . stripslashes(ucwords($currentTerm)) ."</title>";             
            if(lsp_get_option('lsp-biztype')=='ls2') return stripslashes(ucwords($currentTerm)) . __(' near ', 'lsp_text_domain'). stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['city'])))) . ", " . stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['state'])))) . "";    
            else return stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['city'])))) . ", " . stripslashes(ucwords(str_replace("-", " ", urldecode($wp_query->query_vars['state'])))) . " " . stripslashes(ucwords($currentTerm)) . ""; 
...
function lsp_get_content()
{
    ...
    if ((lsp_get_option('lsp-cities-all-pages') != 'none') && ((lsp_get_option('lsp-api-city') != "") && (lsp_get_option('lsp-publish-now') == 'yes' || ((current_user_can('administrator')) && (lsp_get_option('lsp-publish-now') == 'preview'))) && (lsp_get_option('lsp-ptemplate-id') != $post->ID) && (is_home() || is_front_page() || ((lsp_get_option('lsp-cities-all-pages') != 'no'))))) {
        ...
        if (lsp_get_option('lsp-footer-links') != 'no') {
            ...
            if (wp_kses(get_option('lsp_content'),array( 'a' => array( 'href'=>array(),'title'=>array()))))
              return wp_kses(get_option('lsp_content'),array( 'a' => array( 'href'=>array(),'title'=>array())));
...
}else if ($my_theme->get('Name') == 'Twenty Sixteen') {
    add_action( 'fl_after_footer', 'lsp_print_content' );
...
}else if ($my_theme->get('Name') == 'Twenty Seventeen') {
    add_action( 'fl_after_footer', 'lsp_print_content' );
...
function lsp_metas()
{
  ...
  $tester = 0;
    ...
    if(strstr($_SERVER['HTTP_REFERER'],"duckduckgo"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"aol"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"naver"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"daum"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"dogpile"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"ask.com"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"rambler"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"search.com"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"haosou"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"shenma"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"easou"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"youdao"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"sougou"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"soso"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"7search"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"seznam"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"biglobe"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"entireweb"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"lycos"))$tester = 1;
    if(strstr($_SERVER['HTTP_REFERER'],"ecosia"))$tester = 1;
...
function lsp_options()
{
    ...
    if(!(current_user_can('administrator')))die(__("You do not have administrator privileges for this.", 'lsp_text_domain'));

    // consent requirement enforcement
    if($_POST['lsp-agree']=='no')die( __("Acceptance of data-sending back and forth is required to use Best Local SEO Tools.", 'lsp_text_domain') );
    ...
    if (isset($_POST['lsp-post-blog'])) {
     ...
     wp_insert_term($_POST['lsp-services10'],'servicesportfolio');
        ...
        if(sanitize_text_field($_POST['lsp-api-radius']) > 15 && $plus == 0) $lsp_radius = 15;
        ...
        if($_POST['lsp-local-max-limit0'])$lsp_local_max_limit         = sanitize_text_field($_POST['lsp-local-max-limit0']);
        else $lsp_local_max_limit = "100000000";
        if($_POST['lsp-national-max-limit0'])$lsp_national_max_limit      = sanitize_text_field($_POST['lsp-national-max-limit0']);
        else $lsp_national_max_limit = "100000000";
        if($_POST['lsp-international-max-limit0'])$lsp_international_max_limit = sanitize_text_field($_POST['lsp-international-max-limit0']);
        else $lsp_international_max_limit = "100000000";
...
<br>
  ...
  <b><?php
        _e("Local Portfolio SEO Visits This

   Month:", 'lsp_text_domain');
?></b>
<br><?php
        $amount = lsp_wp_httpget("http://www.bestlocalseotools.com/index3.php?request=viewcount&key=" . lsp_get_option('lsp-api-key'));;
        if ($amount < 1)
...
?> <br>
   <input type="text" name="lsp-city" value="<?php
            echo $lsp_city;
?>" size="40"></p>
...
?> <br>
   <input type="text" name="lsp-state" value="<?php
            echo $lsp_state;
?>" size="40"></p>
...
?></font><br> 
  <textarea rows="4" cols="50" name="lsp-intro-text"><?php
            echo $lsp_intro_text;
?></textarea><font color="#aaaaaa" size="-1"><br><?php
...
?></option>
   ...
   <br> <a href="#" onclick="return false;" class="lsp-show-advanced"><?php
...
?></a><br>
       <input type="text" name="lsp-keyphrase-override" value="<?php
...
?> <br>
  <input type="text" name="lsp-local-population-limit" value="<?php
...
?> <br>
    <input type="text" name="lsp-exclude-cities" value="<?php
...
?> <br>
    <input type="text" name="lsp-exclude-countries" placeholder="<?php
...
?> <br>
    <input type="text" name="lsp-add-countries" placeholder="<?php
            _e('English 2 letter country code(s), comma separate multiple', 'lsp_text_domain');
?>" value="<?php
...
?> <br>
  <input type="text" name="lsp-local-focus" value="<?php
...
?> <br>
  <input type="text" name="lsp-country-focus" value="<?php
...
?> <br>
  <input type="text" name="lsp-state-focus" value="<?php
            echo $lsp_state_focus;
?>" size="40">


  <font color='#aaaaaa'><br><br><?php
...
?> <br>
  <input type="text" name="lsp-national-population-limit" value="<?php
...
?> <br>
  <input type="text" name="lsp-international-population-limit" value="<?php
...
?> <br>
  <input type="text" name="lsp-local-max-limit" value="<?php
...
?> <br>
  <input type="text" name="lsp-national-max-limit" value="<?php
...
?> <br>
  <input type="text" name="lsp-international-max-limit" value="<?php
...
<br><a href="http://www.bestlocalseotools.com" target="_blank"><?php
...
?></h3> -->
 ...
 <span class="biztype" id="biztype"><?php
...
?></option>
  ...
  <p><?php
        _e("City (center of service area):", 'lsp_text_domain');
?> <br>
   <input type="text" name="lsp-api-city" value="<?php
        echo $lsp_api_city;
?>" size="40"></p>
  ...
  <p><?php
        _e("State / Province (full state name):", 'lsp_text_domain');
?> <br>
   <input type="text" id="lsp-api-state" name="lsp-api-state" value="<?php
        echo $lsp_api_state;
?>" size="40"></p>
...
<?php
        _e("Radius Served in Miles", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-api-radius" value="<?php
        echo $lsp_api_radius;
?>" size="5">
  </p>
...
?> <br>
   <input type="text" name="lsp-largest-city" value="<?php
        echo $lsp_largest_city;
?>" size="40"></p>
</span>

 <span class="percents" id="percent1"><?php
...
?>" type="text" id="lsp-services" name="lsp-services" value="<?php
        echo $lsp_services;
?>" size="30">  <input type="text" name="lsp-percent" value="<?php
        echo $lsp_percent;
?>" size="4">%
...
?><input type="text" id="lsp-services2" name="lsp-services2" value="<?php
        echo $lsp_services2;
?>" size="30"> <input type="text" name="lsp-percent2" value="<?php
        echo $lsp_percent2;
?>" size="4">%
...
?><input type="text" id="lsp-services3" name="lsp-services3" value="<?php
        echo $lsp_services3;
?>" size="30"> <input type="text" name="lsp-percent3" value="<?php
        echo $lsp_percent3;
?>" size="4">%
...
?><input type="text" id="lsp-services4" name="lsp-services4" value="<?php
        echo $lsp_services4;
?>" size="30"> <input type="text" name="lsp-percent4" value="<?php
        echo $lsp_percent4;
?>" size="4">%
...
?><input type="text" id="lsp-services5" name="lsp-services5" value="<?php
        echo $lsp_services5;
?>" size="30"> <input type="text" name="lsp-percent5" value="<?php
        echo $lsp_percent5;
?>" size="4">%
...
?><input type="text" id="lsp-services6" name="lsp-services6" value="<?php
        echo $lsp_services6;
?>" size="30"> <input type="text" name="lsp-percent6" value="<?php
        echo $lsp_percent6;
?>" size="4">%
...
?><input type="text" id="lsp-services7" name="lsp-services7" value="<?php
        echo $lsp_services7;
?>" size="30"> <input type="text" name="lsp-percent7" value="<?php
        echo $lsp_percent7;
?>" size="4">%
...
?><input type="text" id="lsp-services8" name="lsp-services8" value="<?php
        echo $lsp_services8;
?>" size="30"> <input type="text" name="lsp-percent8" value="<?php
        echo $lsp_percent8;
?>" size="4">%
...
?><input type="text" id="lsp-services9" name="lsp-services9" value="<?php
        echo $lsp_services9;
?>" size="30"> <input type="text" name="lsp-percent9" value="<?php
        echo $lsp_percent9;
?>" size="4">%
...
?><input type="text" id="lsp-services10" name="lsp-services10" value="<?php
        echo $lsp_services10;
?>" size="30"> <input type="text" name="lsp-percent10" value="<?php
        echo $lsp_percent10;
?>" size="4">%
...
?></option>
   ...
   <p><?php
    _e("Also Create a Traditional Portfolio at ", 'lsp_text_domain'); echo "<a href='".home_url()."/ourwork/"."' target='_blank'>".home_url()."/ourwork/</a> "; _e("(you can add service/industry filter widgets)", 'lsp_text_domain');
?> <br>
...
<?php if(lsp_get_option('lsp-agree')=="yes" || lsp_get_option('lsp-agree')=="yes2"):?>
  ...
  <a href="#" id="portfoliosetuplink"><u><?php
...
?> <br>
    <!--
  <textarea rows="4" cols="50" name="lsp-api-intro-textNah"><?php
    echo $lsp_api_intro_text;
?>
  </textarea>-->

  <?php
    wp_editor($lsp_api_intro_text, 'lsp-api-intro-text', array(
        'editor_height' => 100
    ));
?>
  <font color="#aaaaaa" size="-1"><br><?php
    _e('Placeholders of', 'lsp_text_domain');
?> [city] <?php
...
<br>
 ...
 <a href="#" onclick="return false;" id="colorsLink"><?php
...
?></font><br>
    <a href="https://www.webpagefx.com/web-design/color-picker/" target="_blank"><?php
    _e("Footer Text Color:", 'lsp_text_domain');
    ?></a> <br>
    #<input type="text" class="color {required:false}"  name="lsp-text-color" value="<?php
    echo $lsp_text_color;
?>" size="40"></p>
    <p><a href="https://www.webpagefx.com/web-design/color-picker/" target="_blank"><?php
    _e("Link Color:", 'lsp_text_domain');
    ?></a> <br>
    #<input type="text" class="color {required:false}"   name="lsp-link-color" value="<?php
    echo $lsp_link_color;
?>" size="40"></p>
    <p><a href="https://www.webpagefx.com/web-design/color-picker/" target="_blank"><?php
    _e("Text Header Color:", 'lsp_text_domain');
    ?></a> <br>
    #<input type="text" class="color {required:false}"   name="lsp-header-color" value="<?php
    echo $lsp_header_color;
?>" size="40"></p>
    <p><a href="https://www.webpagefx.com/web-design/color-picker/" target="_blank"><?php
    _e("Homepage Footer Links Background Color:", 'lsp_text_domain');
    ?></a> <br>
    #<input type="text" class="color {required:false}"   name="lsp-homepage-footer-links-background-color" value="<?php
...
?> <br>
    <input type="text" name="lsp-add-classes" value="<?php
...
?> <br>
    <input type="text" name="lsp-footer-intro" value="<?php
...
?> <br>
    <input type="text" name="lsp-linking-sentence" value="<?php
...
?> <br>
    <input type="text" name="lsp-served-sentence" value="<?php
...
<br>
<a href="#" onclick="return false;" class="lsp-show-links"><?php
...
<select name='iservice' id='iservice'>
  <option id='service' value='service'><?php
    _e('Service', 'lsp_text_domain');
?></option>
  <option id='industry' value='industry'><?php
...
<br>
 ...
 <a href="#" onclick="return false;" class="lsp-show-advanced"><?php
    _e('Show Advanced Controls', 'lsp_text_domain');
?></a>
...
<div class="lsp-advanced">
  ...
  <p><?php
        _e("Exclude Cities:", 'lsp_text_domain');
?> <br>
    <input type="text" name="lsp-exclude-cities0" value="<?php
...
?>" size="40"></p>
 ...
 <p><?php
        _e("Local Set City Population Minimum (for getting it's own portfolio page):", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-local-population-limit0" value="<?php
        echo $lsp_local_population_limit0;
        ?>" size="40"></p>
  <p><?php
        _e("Local Set City Population Maximum (for getting it's own portfolio page):", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-local-max-limit0" value="<?php
...
?> <br>
  <input type="text" name="lsp-add-cities0" value="<?php
...
?> <br>
  <input type="text" name="lsp-abbreviations0" value="<?php
...
?> <br>
    <input type="text" name="lsp-exclude-states0" value="<?php
...
?> <br>
  <input type="text" name="lsp-add-states9" value="<?php
...
?>
  ...
  <p><?php
        _e("Exclude Countries:", 'lsp_text_domain');
?> <br>
    <input type="text" name="lsp-exclude-countries0" placeholder="<?php
        _e('English 2 letter country code(s), comma separate multiple', 'lsp_text_domain');
?>" value="<?php
...
?>" size="40"></p>
  ...
  <p><?php
        _e("Only These Countries:", 'lsp_text_domain');
?> <br>
    <input type="text" name="lsp-add-countries0" placeholder="<?php
        _e('English 2 letter country code(s), comma separate multiple', 'lsp_text_domain');
?>" value="<?php
...
?></option>
  ...
  <p><?php
        _e("Local Area Percent Focus", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-local-focus0" value="<?php
...
?>" size="40">
  ...
  <p><?php
        _e("Home Country Percent Focus", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-country-focus0" value="<?php
...
?>" size="40">
  ...
  <p><?php
        _e("International Percent Focus", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-state-focus0" value="<?php
...
?></font>
  ...
  <p><?php
        _e("National Set City Population Minimum (for getting it's own portfolio page)", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-national-population-limit0" value="<?php
...
?>" size="40">
  ...
  <p><?php
        _e("International Set City Population Minimum (for getting it's own portfolio page):", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-international-population-limit0" value="<?php
...
?>" size="40">
  ...
  <p><?php
        _e("National Set City Population Maximum (for getting it's own portfolio page)", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-national-max-limit0" value="<?php
...
?>" size="40">
  ...
  <p><?php
        _e("International Set City Population Maximum (for getting it's own portfolio page):", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-international-max-limit0" value="<?php
...
<?php
    ...
    if ($plus == 0):
?>
<br><br><a href="http://www.bestlocalseotools.com" target="_blank"><?php
        _e('Upgrade to Premium at bestlocalseotools.com', 'lsp_text_domain');
?></a> <?php
        _e('to get the full version and unlock additional features.', 'lsp_text_domain');
?> 
...
?></a>
  <br><?php endif;?>
  <?php if(lsp_get_option('lsp-agree')=="yes" || lsp_get_option('lsp-agree')=="yes2"):?>
<a href="#" id="reviewssetuplink"><u><?php
...
?> <br>
  <input type="text" name="lsp-reviews-name" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url" value="<?php
            echo $lsp_reviews_url;
?>" size="40">
<br>

<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name2" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url2" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc2" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name3" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url3" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc3" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name4" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url4" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc4" value="<?php
            echo $lsp_reviews_loc4;
?>" size="40">
<br></span>

<a href="#" id="showMoreReviews"><?php
...
?> <br>
  <input type="text" name="lsp-reviews-name5" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url5" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc5" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name6" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url6" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc6" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name7" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url7" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc7" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name8" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url8" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc8" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name9" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url9" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc9" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name10" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url10" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc10" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name11" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url11" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc11" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name12" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url12" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc12" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name13" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url13" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc13" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name14" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url14" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc14" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name15" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url15" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc15" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name16" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url16" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc16" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name17" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url17" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc17" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name18" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url18" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc18" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name19" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url19" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc19" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name20" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url20" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc20" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name21" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url21" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc21" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name22" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url22" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc22" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name23" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url23" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc23" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name24" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url24" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc24" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name25" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url25" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc25" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name26" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url26" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc26" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name27" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url27" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc27" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name28" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url28" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc28" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name29" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url29" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc29" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-name30" value="<?php
...
?> <br>
  <input type="text" name="lsp-reviews-url30" value="<?php
...
<span class="storeLocation"><?php
            _e("for Store Location Named:", 'lsp_text_domain');
?> <br>
  <input type="text" name="lsp-reviews-loc30" value="<?php
...
<a href="#" id="showMoreReviewOptions"><?php
...
?> <br>
  <input type="text" name="lsp-captcha-question" value="<?php
...
?> <br>
  <input type="text" name="lsp-captcha-answer" value="<?php
...
?> <br>
<input type="text" name="lsp-addressName1" value="<?php
...
?> <br>
<input type="text" name="lsp-addressURL1" value="<?php
...
?> <br>
<input type="text" name="lsp-address1" value="<?php
...
?> <br>
<input type="text" name="lsp-addressLocality1" id="city1" value="<?php
...
?> <br>
<input type="text" name="lsp-addressRegion1" id="state1" value="<?php
...
?> <br>
<input type="text" name="lsp-postalCode1" value="<?php
...
?> <br>
<input type="text" name="lsp-addressCountry1" id="country1" placeholder="US" value="<?php
...
?> <br>
<input type="text" name="lsp-addressTelephone1" value="<?php
...
?> <br>
<input type="text" name="lsp-addressImageURL1" value="<?php
...
?> <br>
<input type="text" name="lsp-addressBusinessImageURL1" value="<?php
...
?> <br>
<input type="text" name="lsp-addressIDURL1" value="<?php
...
<!--
 <input type="text" class="dateSelector" name="lsp-locationDate1" value="<?php
        echo $term['lsp-locationDate1'];
?>" size="12"> <?php
        _e(" - ", 'lsp_text_domain');
?>
 <input type="text" name="lsp-locationDate2" class="dateSelector" value="<?php
...
?>" name="lsp-archive-types" value="<?php
...
<select name="lsp-success" id="lsp-success">
    <option value="conversions" <?php if($lsp_success=="conversions")echo "selected"; ?>><?php _e('Conversions (with Pageviews as Tiebreaker)','lsp_text_domain');?></option>
    <option value="pageviews" <?php if($lsp_success=="pageviews")echo "selected"; ?>><?php _e('Pageviews from SEO','lsp_text_domain');?></option>
    <option value="both" <?php if($lsp_success=="both")echo "selected"; ?>><?php _e('Both Conversions & Pageviews from SEO to Maximize Value Per My Settings','lsp_text_domain');?></option>
  </select>
<br>
...
<select name="lsp-newerolder">
    <option value="newer" <?php if($lsp_newerolder=="newer")echo "selected"; ?>><?php _e('Newer','lsp_text_domain');?></option>
    <option value="older" <?php if($lsp_newerolder=="older")echo "selected"; ?>><?php _e('Older','lsp_text_domain');?></option>
  </select>
...
?> <br>
  <input type="text" name="lsp-mozrank" value="<?php
    echo $lsp_mozrank;
?>" size="40">
-->
...
?>
 <span class="scores" id="score1"><b><?php
...
<?php endif;?>
  ...
  <input type="submit" name="Submit" class="button-primary" id="submitter" value="<?php
...
function lsp_activate()
{
    ...
    if(lsp_get_option('lsp-split1')=="")update_option( 'lsp-split1', "60" , "yes");
    if(lsp_get_option('lsp-split2')=="")update_option( 'lsp-split2', "30" , "yes");
    if(lsp_get_option('lsp-split3')=="")update_option( 'lsp-split3', "30" , "yes");
    if(lsp_get_option('lsp-split4')=="")update_option( 'lsp-split4', "1" , "yes");
  ...
  if(lsp_get_option('lsp-percent')=="")update_option('lsp-percent','100',"yes");
  if(lsp_get_option('lsp-percent2')=="")update_option('lsp-percent2','0',"yes");
  if(lsp_get_option('lsp-percent3')=="")update_option('lsp-percent3','0',"yes");
  if(lsp_get_option('lsp-percent4')=="")update_option('lsp-percent4','0',"yes");
  if(lsp_get_option('lsp-percent5')=="")update_option('lsp-percent5','0',"yes");
  if(lsp_get_option('lsp-percent6')=="")update_option('lsp-percent6','0',"yes");
  if(lsp_get_option('lsp-percent7')=="")update_option('lsp-percent7','0',"yes");
  if(lsp_get_option('lsp-percent8')=="")update_option('lsp-percent8','0',"yes");
  if(lsp_get_option('lsp-percent9')=="")update_option('lsp-percent9','0',"yes");
  if(lsp_get_option('lsp-percent10')=="")update_option('lsp-percent10','0',"yes");
  if(lsp_get_option('lsp-alt-filter-all')=="")update_option('lsp-alt-filter-all','yes',"yes");
...
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" name="lsp_post_client_name" value='<?php
...
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" id="lsp_post_testimonial" name="lsp_post_testimonial" value='<?php
...
?>:</td><td style="width:100%">
      <input type='text' style="width:100%;min-width:200px" name="lsp_post_city" value='<?php
    echo $lsp_post_city;
?>' /></td>
    </tr>
    <tr>
    ...
    _e("State / Province (full state name)*", "lsp_text_domain");
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" id="lsp_post_state" name="lsp_post_state" value='<?php
    echo $lsp_post_state;
?>' /></td>
...
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" name="lsp_post_score" value='<?php
    echo $lsp_post_score;
?>' /></td>
    </tr>
...
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" name="lsp_post_workedwith" value='<?php
...
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" name="lsp_post_storelocation" value='<?php
...
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" name="lsp_post_client_website" value='<?php
...
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" name="lsp_post_client_phone" value='<?php
...
?>:</td><td>
      <input type='text' style="width:100%;min-width:200px" name="lsp_post_email" value='<?php
    echo $lsp_post_email;
?>' /></td>
    </tr>
...
//  if(get_option('lsp-show-all-projects')!="yes") $queryStart .= " AND d.meta_value LIKE '%%%s%%' ";
  $query = $wpdb->prepare("$queryStart AND lsp_lp_tags LIKE '%%%s%%'
  AND lsp_lp_latitude >= %f 
  AND lsp_lp_latitude <= %f 
  AND lsp_lp_longitude >= %f 
  AND lsp_lp_longitude <= %f", $keyPhrase, ($rangerLat - (0/52)), ($rangerLat + (0/52)), 
  ($rangerLong - (0/52)), ($rangerLat + ($rangerLong/52))
  );
//  echo $query;
//  echo "hello";
 ...
 //else $result = mysql_query("SELECT * FROM ( SELECT geoname.name as city, admin1CodesAscii.name as state, geoname.population as population, (3959 * acos( cos( radians($latitude) ) * cos( radians( geoname.latitude) ) * cos( radians( geoname.longitude) - radians($longitude) ) +sin(radians($latitude)) * sin(radians(geoname.latitude)) ) ) AS distance FROM geoname, admin1CodesAscii WHERE geoname.admin1 = admin1CodesAscii.regionCode AND geoname.latitude >". ($latitude - $adjustedRadius) ." AND geoname.latitude <". ($latitude + $adjustedRadius) ." AND geoname.longitude >". ($longitude - $adjustedRadius) ." AND geoname.longitude <". ($longitude + $adjustedRadius) ." ORDER BY distance LIMIT 0,".$count. ") as geotable ORDER BY geotable.city");
  ...
  if(get_option('lsp-show-all-projects')!="yes") $queryString .= " AND lsp_lp_tags LIKE '%%%s%%' ";
...
", $keyword));
    ...
    if (lsp_get_option('lsp-hide-intro-text') != 'yes') {
        if ($phraseNumber == 1)
            $holder_string = wp_kses(get_option('lsp-api-intro-text'),array( 'a' => array( 'href'=>array(),'title'=>array()) )) . "<br><br>";
        if ($phraseNumber == 2)
            $holder_string = wp_kses(get_option('lsp-api-intro-text'),array( 'a' => array( 'href'=>array(),'title'=>array()) )) . "<br><br>";
        if ($phraseNumber == 3)
            $holder_string = wp_kses(get_option('lsp-api-intro-text'),array( 'a' => array( 'href'=>array(),'title'=>array()) )) . "<br><br>";
...
class lsp_fp_widget extends WP_Widget
{
    ...
    {
        parent::__construct('lspfp_widget', __('Featured Posts', 'lsp_text_domain'), array(
...
class lsp_hp_widget extends WP_Widget
{
    ...
    function __construct()
    {
        parent::__construct('lsphp_widget', __('Popular Products', 'lsp_text_domain'), array(
...
class lsp_sp_widget extends WP_Widget
{
    ...
    function __construct()
    {
        parent::__construct('lspsp_widget', __('Portfolio by Service', 'lsp_text_domain'), array(
...
class lsp_ip_widget extends WP_Widget
{
    ...
    function __construct()
    {
        parent::__construct('lspip_widget', __('Portfolio by Industry', 'lsp_text_domain'), array(
...
function lsp_sp_widget_content()
{
    ...
    $terms = get_terms(array('taxonomy'=>'servicesportfolio', 'hide_empty' => false));
...
function lsp_ip_widget_content()
{
    ...
    $terms = get_terms(array('taxonomy'=>'industriesportfolio', 'hide_empty' => false));
    foreach($terms as $term){
...
function lsp_metacontent($content){
  ...
  if(get_post_type() == "localproject") {
    ...
    $returnStr .= __("Client Name: ", "lsp_text_domain") . lsp_get_post_meta($thispostid, 'lsp_post_client_name', true) . "";
        ...
        $returnStr .= "<br>" . __("City: ", "lsp_text_domain") . lsp_get_post_meta($thispostid, 'lsp_post_city', true);
        $returnStr .= "<br>" . __("State: ", "lsp_text_domain") . lsp_get_post_meta($thispostid, 'lsp_post_state', true);
        if(lsp_get_option('lsp-biztype')!="ec")$returnStr .= "<br>" . __("Services Used: ", "lsp_text_domain") . str_replace(',',', ',ucwords(lsp_get_post_meta($thispostid, 'lsp_post_tags', true)));
        else $returnStr .= "<br>" . __("Products / Services Provided: ", "lsp_text_domain") . str_replace(',',', ',ucwords(lsp_get_post_meta($thispostid, 'lsp_post_tags', true)));
...
function lsp_get_redirect($oldurl){
  ...
  if($results){
    foreach($results as $result){
      if($result->new_url)$holder = $result->new_url;
    }    
  }
  return $holder;
}
...
function lsp_redirects_activate() {
  ...
  if(get_option('lsp-schema-updated')!="yes")lsp_update_old_schema();
...
function lsp_deactivate(){
  ...
  $results = $wpdb->get_results($query2);
    ...
    foreach($results as $post){
      ...
      if(get_post_meta($thispostid, '_focuskwold') !="" && get_post_meta($thispostid, '_focuskwold') !="lsp no value") wpseo_set_value('focuskw',get_post_meta($thispostid, '_focuskwold'),$thispostid);
      if(get_post_meta($thispostid, '_focuskwold') =="lsp no value") wpseo_set_value('focuskw','',$thispostid);

      if(get_post_meta($thispostid, '_titleold') !="" && get_post_meta($thispostid, '_titleold') !="lsp no value") wpseo_set_value('title',get_post_meta($thispostid, '_titleold'),$thispostid);
      if(get_post_meta($thispostid, '_titleold') =="lsp no value") wpseo_set_value('title','',$thispostid);

      if(get_post_meta($thispostid, '_metadescold') !="" && get_post_meta($thispostid, '_metadescold') !="lsp no value") wpseo_set_value('metadesc',get_post_meta($thispostid, '_metadescold'),$thispostid);
      if(get_post_meta($thispostid, '_metadescold') =="lsp no value") wpseo_set_value('metadesc','',$thispostid);

      if(get_post_meta($thispostid, '_metakeyold') !="" && get_post_meta($thispostid, '_metakeyold') !="lsp no value") wpseo_set_value('metakey',get_post_meta($thispostid, '_metakeyold'),$thispostid);
      if(get_post_meta($thispostid, '_metakeyold') =="lsp no value") wpseo_set_value('metakey','',$thispostid);

      if(get_post_meta($thispostid, '_focuskeywordsold',true) != "" ) update_post_meta($thispostid, '_yoast_wpseo_focuskeywords', get_post_meta($thispostid,'_focuskeywordsold',true) );
...
if($termsArray1) $termsArrayHolder = array_merge($termsArray1,$termsArray2);
if($termsArray2) $termsArrayHolder = array_merge($termsArrayHolder,$termsArray3);
if($termsArray3) $termsArrayHolder = array_merge($termsArrayHolder,$termsArray4);
if($termsArray4) $termsArrayHolder = array_merge($termsArrayHolder,$termsArray5);
if($termsArray5) $termsArrayHolder = array_merge($termsArrayHolder,$termsArray6);
if($termsArray6) $termsArrayHolder = array_merge($termsArrayHolder,$termsArray7);
if($termsArray7) $termsArrayHolder = array_merge($termsArrayHolder,$termsArray8);
if($termsArray8) $termsArrayHolder = array_merge($termsArrayHolder,$termsArray9);
if($termsArray9) $termsArrayHolder = array_merge($termsArrayHolder,$termsArray10);

$suggestionsString = "";
  if(is_array($termsArrayHolder))foreach($termsArrayHolder as $term){
    if(strstr($term[0],$focusholder)) $suggestionsString .= $term[0]."<br>";
  }

?>


<?php if($suggestionsString):
$haveAny = 1;
?>
<br>
...
<?php endif; ?>
 ...
 <b><?php
$haveAny = 1;
  _e("Consider Adding a Page Focused On:","lsp_text_domain");?></b><br>
 Do E<br>
<?php endif; ?>
